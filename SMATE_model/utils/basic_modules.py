...
import tensorflow.compat.v1.keras.layers as ll
import random as rd 
import numpy as np
...
#for ablation tests
def encoder2_smate(in_shape, pool_step, d_prime, kernels = [8, 5, 3]):
    input_ = Input(shape=in_shape)  # input: (samples, L, input_dims)
    L = in_shape[0]
    ...
    out = ll.Concatenate(axis=-1)([out_t]) # (samples, L', 128*4 + 128)
    print(out.shape)
    out = Dense(128)(out)
    #out = Dense(128)(out_s)
    out = BatchNormalization()(out)
...
#for ablation tests
def decoder2_smate(encoder, timesteps, data_dim, pool_step):
    input_ = encoder.output  # input: (batch_size, timesteps, latent_dim)
