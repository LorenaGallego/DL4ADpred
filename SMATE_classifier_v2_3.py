...
def get_parameters():
        ...
        parser.add_argument('--outModelFile', type=str, default='./out/REM/best_model_REM_sk',
...
if __name__ == "__main__":
    ...
    X = np.load('Datasets/MTS-UEA/' + ds_name + '/X_REM.npy')
    y = np.load('Datasets/MTS-UEA/' + ds_name + '/y_REM.npy')
    IDs = np.load('Datasets/MTS-UEA/' + ds_name + '/IDs_REM.npy')
    ...
    for fold, (train_index, test_index) in enumerate(cv.split(np.zeros(len(X_patients)), y_unique)):
        ...
        train_index, val_index = train_test_split(train_index, test_size=0.1, random_state=0, stratify=y_unique)
        ...
        if fold ==3:
           ...
           # nombre_original = 'original_graph_N3_2'
           # nombre_encoder = 'encoder_graph_N3_2'
           # titulo = '-N3-'
           # smate.tSNE_visual(x_test,y_test,nombre_original,nombre_encoder,titulo)
            #smate.predict_ssl(x_sup, y_sup, x_unsup, y_unsup, x_test, y_test) #semi-supervised predictio
        else: 
            pass
        '''
            current_epoch =0
    ...
    '''
            ...
            print('==== Training ====')
            smate.fit(n_epochs, x_train, x_sup, x_unsup, val_ratio, patience, current_epoch)
            print("training time is {}".format(time.time() - t1))
            smate.model.load_weights(outModelFile_Fold)
