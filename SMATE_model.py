...
class SMATE:
    ...
    def tSNE_visual(self,X,Y,nombre_original,nombre_encoder,titulo):
        ...
        legend_map = {0: 'AD',
              1: 'Healthy'}
        sns.scatterplot(x="dim1", y="dim2", hue=df.labels.map(legend_map),palette=sns.color_palette("hls", 2),data=df).set(title="Original" + titulo + "T-SNE projection")
        plt.xlim(-6,10)
        plt.ylim(-10,10)
        plt.savefig(nombre_original) 
        ...
        plt.figure()
        sns.scatterplot(x="dim1", y="dim2", hue=df.labels.map(legend_map),palette=sns.color_palette("hls", 2),data=df2).set(title="Representation space" + titulo + "T-SNE projection")
        plt.xlim(-70,70)
        plt.ylim(-60,60)
        plt.savefig(nombre_encoder)
