...
from HMM_att_model import ATT_model
from HMM_att_utils import *
...
parser.add_argument('--sup_ratio', type=float, default=0.0,
...
parser.add_argument('--kernels', type=str, default="3,2,1",
...
parser.add_argument('--n_sts', type=int, default=5,
                            help='Number of states for the HMM training')
parser.add_argument('--n_em', type=int, default=4,
                    help='Number of emissions for the HMM training')
parser.add_argument('--outModelFile', type=str, default='./out/trainer_HMM_feat2_st5_model2_REM',
...
def test(fold):
    ...
    fpr, tpr, th = roc_curve(labels[indices_test].detach().cpu().numpy(), output[192:][:,1].detach().cpu().numpy())
    print(fpr.shape)
    print(tpr.shape)
    #print(th)
...
#load the data processed by HMM model
out = './out/trainer_HMM_feat2_st5_model2_REM'
features_fold, labels_fold, idx_train_fold, idx_val_fold, idx_test_fold, nclass_fold = load_raw_ts(args.data_path, dataset=args.dataset, fase=args.fase, n_states= 5, n_emissions = 4, out = out)

#train the att model for an unsupervised prediction 
n_folds = 5
...
for i in range(n_folds):
    ...
    model = ATT_model(nfeat=features.shape[1],
